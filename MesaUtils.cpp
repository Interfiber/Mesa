...
std::vector<std::string> Mesa::Util_SplitString(const std::string& input, const std::string& delimiter)
{
...
std::string Mesa::Util_EvalString(std::shared_ptr<Mesa::Workspace> workspace, const std::string& string)
{
    std::string data;
    ...
    // If we encounter no space by the end of the eval just append the variable content
    if (writeVarName) {
        if (workspace->variables.find(varName) == workspace->variables.end()) {
            LOG("Failed to eval statement: No such variable with name: %s\n", varName.c_str());
